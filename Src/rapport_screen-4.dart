  ...
  Future<void> _generateAndShare() async {
    ...
    try {
      ...
      for (int i = ranked.length - 1; i >= ranked.length - 5 && i >= 0; i--) {
        ...
        final int tot = (c.cumulRetraits + (newByCode[c.codeCentre] ?? 0)).toInt();
        final p = c.inscrits > 0 ? tot / c.inscrits * 100 : 0.0;
        bottom5 = '<tr><td style="text-align:center">⚠️</td>'
      ...
      for (final entry in arrMap.entries) {
        ...
        <tr style="background:#eff6ff">
          <td colspan="6" style="font-weight:700;color:#1d4ed8;padding:7px 8px">
            🏙️ ${entry.key} — ${_fmt(arrRet)} / ${_fmt(arrIns)} = <span style="color:$col">${arrPct.toStringAsFixed(1)}%</span>
          </td>
        </tr>''';
...
<title>Rapport Retraits — $dateStr</title>
<style>
...
<div class="cover">
  ...
  <div style="font-size:18px;font-weight:800;color:#1565C0;margin-top:6px">ÉLECTIONS PRÉSIDENTIELLES 2026</div>
  <div style="color:#374151;margin-top:4px">Rapport de suivi — Retrait des cartes électorales</div>
  <div style="color:#6b7280;font-size:10px;margin-top:6px">Données au <strong>$dateStr</strong></div>
</div>
<div class="kpi-row">
  ...
  <div class="kpi" style="border-top-color:#16a34a"><div style="font-size:8px;color:#6b7280;text-transform:uppercase">Retirées</div><div style="font-size:16px;font-weight:800;color:#16a34a">${_fmt(totalRet)}</div></div>
...
<div class="grid">
  <div class="box">
    <div class="box-head-green">🏆 TOP 5 — Meilleurs centres</div>
    <table><thead><tr><th>#</th><th>Centre</th><th>Arr.</th><th style="text-align:right">Taux</th></tr></thead>
    <tbody>$top5</tbody></table>
  </div>
  <div class="box">
    <div class="box-head-red">🚨 RETARDATAIRES</div>
    <table><thead><tr><th>#</th><th>Centre</th><th>Arr.</th><th style="text-align:right">Taux</th></tr></thead>
    <tbody>$bottom5</tbody></table>
  </div>
</div>
<div class="section-title">Détail par centre</div>
<table><thead><tr>
...
<div style="text-align:center;font-size:9px;color:#9ca3af;margin-top:16px;padding-top:8px;border-top:1px solid #e5e7eb">
  Élections 2026 — Djibouti | $dateStr | 162 833 inscrits · 413 bureaux · 39 centres
</div>
</body></html>''';
      ...
      body: SingleChildScrollView(
        ...
        child: Column(children: [
          Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(children: [
            const Text('📊 Rapport de retrait des cartes', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            const SizedBox(height: 12),
            Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
              Column(children: [Text(_fmt(totalRet), style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFF16a34a))), const Text('Retirées', style: TextStyle(fontSize: 11, color: Colors.grey))]),
          ...
          SizedBox(width: double.infinity, child: ElevatedButton.icon(
            ...
            label: Text(_generating ? 'Génération en cours...' : '📤 Générer et partager le rapport HTML'),
          ...
          const Text('Le rapport s\'ouvre dans votre navigateur ou peut être partagé par email/WhatsApp', style: TextStyle(fontSize: 10, color: Colors.grey), textAlign: TextAlign.center),
        ]),
      ),
    );
  }

...
class _SyncScreenState extends State<SyncScreen> {
  ...
  Future<void> _loadDates() async {
    // Utilise DbHelper.all() — la méthode existante
  ...
  Future<void> _exportCsv({String? date}) async {
    ...
    try {
      // Utilise DbHelper.all() — la seule méthode disponible
      final saisies = await DbHelper.all();
      final filtered = date != null
          ? saisies.where((s) => s.date == date).toList()
          : saisies;

      if (filtered.isEmpty) {
        setState(() {
          _status = 'Aucune donnée${date != null ? " pour $date" : ""}';
          _statusOk = false;
        });
        return;
      }
      ...
      for (final s in filtered) {
        final nom = s.nomCentre.replaceAll(',', ' ');
        sb.writeln('${s.codeCentre},$nom,${s.date},${s.retraits},${s.arrondissement}');
      }
      ...
      setState(() {
        _status = '✅ CSV partagé — ${filtered.length} lignes (${date ?? "toutes dates"})';
        _statusOk = true;
      });
    } catch (e) {
        ...
        children: [
          ...
          Container(
            ...
            child: const Column(children: [
              ...
              SizedBox(height: 10),
              Text('EXPORT CSV', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w900, letterSpacing: 1)),
              SizedBox(height: 4),
              Text('Exporter les retraits · Partager via WhatsApp / Email / Drive',
          ...
          // Sélection date
          const Text('CHOISIR UNE DATE', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: Color(0xFF6B7280), letterSpacing: 0.8)),
          ...
          Container(
            ...
            child: Column(children: [
              RadioListTile<String?>(
                value: null, groupValue: _selectedDate,
                onChanged: (v) => setState(() => _selectedDate = v),
                title: const Text('Toutes les dates', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13)),
                subtitle: Text('${_dates.length} jour(s) disponibles', style: const TextStyle(fontSize: 11)),
                activeColor: const Color(0xFF059669), dense: true,
              ),
              ..._dates.expand((d) => [
                const Divider(height: 1),
                RadioListTile<String?>(
                  value: d, groupValue: _selectedDate,
                  onChanged: (v) => setState(() => _selectedDate = v),
                  title: Text(d, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13)),
                  subtitle: const Text('Export du jour', style: TextStyle(fontSize: 11)),
                  activeColor: const Color(0xFF059669), dense: true,
                ),
              ]),
            ]),
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              onPressed: _loading ? null : () => _exportCsv(date: _selectedDate),
              icon: _loading
                  ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                  : const Icon(Icons.share, size: 20),
              label: Text(
                _loading ? 'Génération...' : _selectedDate != null ? 'Exporter $_selectedDate' : 'Exporter tout',
          ...
          Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('💡 Comment ça marche', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 12, color: Color(0xFF0C4A6E))),
              const SizedBox(height: 10),
              _step('1', 'Sélectionnez la date (ou toutes)'),
              _step('2', 'Appuyez sur "Exporter"'),
              _step('3', 'Choisissez WhatsApp, Email, Drive…'),
              _step('4', 'Intégrez le fichier CSV dans le tableau de bord'),
            ]),
          ),
          ...
          Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('📄 Format du fichier', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 12, color: Color(0xFF374151))),
              const SizedBox(height: 8),
              Container(
  ...
  Widget _step(String n, String text) => Padding(
    ...
    child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Container(width: 20, height: 20,
        ...
        child: Text(n, style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w700))),
      const SizedBox(width: 8),
      Expanded(child: Text(text, style: const TextStyle(fontSize: 11, color: Color(0xFF1E40AF)))),
    ]),
  );
}

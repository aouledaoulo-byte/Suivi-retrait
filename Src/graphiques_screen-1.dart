  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.end, children: [
          ...
          GestureDetector(
            ...
            child: const Row(children: [
              Icon(Icons.refresh, size: 16, color: Color(0xFF1565C0)),
              SizedBox(width: 2),
              Text('Actualiser', style: TextStyle(fontSize: 11, color: Color(0xFF1565C0))),
            ]),
          ),
        ]),
      ),
      TabBar(
        ...
        labelColor: const Color(0xFF1565C0), unselectedLabelColor: Colors.grey,
...
class _HBarChart extends StatelessWidget {
  ...
  const _HBarChart({required this.values, required this.labels, required this.colors,
    required this.title, this.subtitle = '', this.barWidth = 48});
...
class _BarPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < values.length; i++) {
      ...
      canvas.drawRRect(
        RRect.fromRectAndCorners(Rect.fromLTWH(x, mt + h - barH, bw, barH),
          topLeft: const Radius.circular(3), topRight: const Radius.circular(3)),
        Paint()..color = color,
      );
      tp.text = TextSpan(text: values[i].toStringAsFixed(values[i] < 10 ? 1 : 0),
          style: const TextStyle(fontSize: 9, color: Colors.black87, fontWeight: FontWeight.bold));
      tp.layout();
      if (barH > 12) canvas.drawRect(
        Rect.fromLTWH(x + bw / 2 - tp.width / 2 - 1, mt + h - barH - tp.height - 2, tp.width + 2, tp.height + 2),
        Paint()..color = Colors.white.withOpacity(0.7));
      tp.paint(canvas, Offset(x + bw / 2 - tp.width / 2, mt + h - barH - tp.height - 2));
      tp.text = TextSpan(text: labels[i], style: const TextStyle(fontSize: 9, color: Colors.grey));
      tp.layout();
      canvas.save();
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      if (_sel == null)
        ...
        const Expanded(child: Center(child: Text('Aucune donnée pour ce centre', style: TextStyle(color: Colors.grey))))
...
// ══════════════════════════════════════════════
// ONGLET ARRONDISSEMENTS — FIX LIVE
// ══════════════════════════════════════════════
...
class _ArrGraphState extends State<_ArrGraph> {
  ...
  // ── Calcul live : base + SQLite ──────────────────────────────
  Map<String, _ArrLive> _computeLive() {
    final Map<String, _ArrLive> map = {};
    // Base depuis centresSuivi
    for (final c in ElectionData.centresSuivi) {
      map.putIfAbsent(c.arrondissement, () => _ArrLive(inscrits: 0, retires: 0));
      map[c.arrondissement]!.inscrits += c.inscrits;
      map[c.arrondissement]!.retires  += c.cumulRetraits;
    }
    // Ajout saisies SQLite
    for (final e in widget.saisies) {
      if (map.containsKey(e.arrondissement)) {
        map[e.arrondissement]!.retires += e.retraits;
      }
    }
    return map;
  }
  ...
  String _latestDate() {
    ...
    for (final c in ElectionData.centresSuivi) allDates.addAll(c.retraitsDailyData.keys);
  ...
  Widget _buildBar() {
    final live = _computeLive();
    // Trier par taux décroissant
    final entries = live.entries.toList()
      ..sort((a, b) => b.value.pct.compareTo(a.value.pct));

    final labels = entries.map((e) => e.key
        .replaceAll('arrondissement', 'Arr')
        .replaceAll('Arrondissement', 'Arr')
        .replaceAll('du Plateau', 'Plateau')).toList();
    ...
    return DefaultTabController(length: 2, child: Column(children: [
      ...
      Expanded(child: TabBarView(children: [
        // ── % retrait LIVE ──
        _HBarChart(
          values: entries.map((e) => e.value.pct).toList(),
          labels: labels,
          colors: kPalette.toList(),
          title: 'Taux de retrait par arrondissement (%)',
          subtitle: 'Données au ${_latestDate()} (base + saisies locales)',
        ),
        // ── Nb retirés LIVE ──
        _HBarChart(
          values: entries.map((e) => e.value.retires.toDouble()).toList(),
          labels: labels,
          colors: kPalette.toList(),
...
// Classe utilitaire pour accumulation arrondissement
class _ArrLive {
  int inscrits;
  int retires;
  _ArrLive({required this.inscrits, required this.retires});
  double get pct => inscrits > 0 ? retires / inscrits * 100 : 0;
}
      ...
      if (_selCentre == null)
        ...
        Expanded(child: TabBarView(children: [
          _HBarChart(
            ...
            labels: labels, colors: kPalette.toList(),
            title: 'Votants par bureau — $_selCentre',
            subtitle: '${filtered.length} bureaux • ${filtered.fold(0, (s, b) => s + b.votants)} votants total',
            barWidth: 36,
          ),
          _PieWidget(values: filtered.map((b) => b.votants.toDouble()).toList(), labels: labels, title: 'Répartition — $_selCentre'),
        ])),
      ]))),
    ]);
  }
...
class _PiePainter extends CustomPainter {
  ...
  double _cos(double a) {
    ...
    double s = 1, t = 1;
    for (int i = 1; i <= 8; i++) { t *= -a * a / (2 * i * (2 * i - 1)); s += t; }
    return s;
  }

...
// ══════════════════════════════════════════════
// CHANGEMENT 1 : classe helper pour accumulation
// ══════════════════════════════════════════════
class _ArrData {
  int inscrits = 0;
  int retires  = 0;
...
// ══════════════════════════════════════════════
// CHANGEMENT 2 : _ArrStats calculé dynamiquement
// ══════════════════════════════════════════════
class _ArrStats {
  final String arrondissement;
  final int inscrits;
  final int retires;
  int get restant => inscrits - retires;
  double get pctRetrait => inscrits > 0 ? retires / inscrits : 0;
  const _ArrStats({required this.arrondissement, required this.inscrits, required this.retires});
}

// ══════════════════════════════════════════════
// CHANGEMENT 3 : _LiveStats inclut syntheseArr
// ══════════════════════════════════════════════
class _LiveStats {
  ...
  final int nouveauxRetraits;
  final String dateMaj;
  final List<_CentreStats> centres;
  final List<_ArrStats> syntheseArr; // ← NOUVEAU
  const _LiveStats({required this.totalInscrits, required this.totalRetires, required this.totalRestant,
    required this.pctRetrait, required this.nouveauxRetraits, required this.dateMaj,
    required this.centres, required this.syntheseArr});
}
...
class _CentreStats {
  ...
  final String arrondissement;
  final int inscrits;
  final int retraitBase;
  final int retraitNew;
  int get totalRetraits => retraitBase + retraitNew;
  double get pct => inscrits > 0 ? totalRetraits / inscrits : 0;
  const _CentreStats({required this.codeCentre, required this.nomCentre, required this.arrondissement,
    required this.inscrits, required this.retraitBase, required this.retraitNew});
}
...
class _DashboardTabState extends State<DashboardTab> with AutomaticKeepAliveClientMixin {
  ...
  Future<void> _refresh() async {
    ...
    // ══════════════════════════════════════════
    // CHANGEMENT 4 : calculer syntheseArr LIVE
    // depuis centres (base + SQLite), pas statique
    // ══════════════════════════════════════════
    final Map<String, _ArrData> arrMap = {};
    for (final centre in centres) {
      arrMap.putIfAbsent(centre.arrondissement, () => _ArrData());
      arrMap[centre.arrondissement]!.inscrits += centre.inscrits;
      arrMap[centre.arrondissement]!.retires  += centre.totalRetraits;
    }
    final syntheseArr = arrMap.entries
        .map((e) => _ArrStats(
              arrondissement: e.key,
              inscrits: e.value.inscrits,
              retires:  e.value.retires,
            ))
        .toList()
      ..sort((a, b) => b.pctRetrait.compareTo(a.pctRetrait));
    // ══════════════════════════════════════════
    ...
    setState(() {
      _stats = _LiveStats(
        totalInscrits: totalInscrits, totalRetires: totalRetires,
        totalRestant: totalInscrits - totalRetires,
        pctRetrait: totalRetires / totalInscrits,
        nouveauxRetraits: nouveaux, dateMaj: dateMaj,
        centres: centres,
        syntheseArr: syntheseArr, // ← NOUVEAU
      );
      _loading = false;
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Container(
            ...
            child: Column(children: [
              ...
              Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                const SizedBox(width: 8),
                Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: Colors.white24, borderRadius: BorderRadius.circular(4)), child: const Text('/ 85% objectif', style: TextStyle(color: Colors.white70, fontSize: 10))),
              ]),
              const SizedBox(height: 10),
              Stack(children: [
                Container(height: 14, decoration: BoxDecoration(color: Colors.white24, borderRadius: BorderRadius.circular(7))),
                FractionallySizedBox(widthFactor: 0.85, child: Container(height: 14, decoration: BoxDecoration(color: Colors.white30, borderRadius: BorderRadius.circular(7)))),
          ...
          if (s.nouveauxRetraits > 0) ...[
            ...
            ...topCentres.where((c) => c.retraitNew > 0).map((c) {
              ...
              Color cc = pctC >= 40 ? Colors.green : pctC >= 25 ? Colors.orange : Colors.red;
          ...
          // ── Synthèse arrondissements LIVE (s.syntheseArr) ──
          const Text('Synthèse arrondissements', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
          const SizedBox(height: 6),
          ...s.syntheseArr.map((a) {
            final pctA = a.pctRetrait * 100;
            Color ca = pctA >= 40 ? Colors.green : pctA >= 25 ? Colors.orange : Colors.red;
            final label = a.arrondissement
                .replaceAll('arrondissement', 'Arr.')
                .replaceAll('Arrondissement', 'Arr.');
            return Padding(
              ...
              child: Row(children: [
                ...
                Expanded(flex: 5, child: LinearProgressIndicator(
                    value: a.pctRetrait.clamp(0, 1),
                    backgroundColor: Colors.grey[200], color: ca, minHeight: 8)),
                const SizedBox(width: 6),
                SizedBox(width: 46, child: Text('${pctA.toStringAsFixed(2)}%',
                    style: TextStyle(fontSize: 11, color: ca, fontWeight: FontWeight.bold),
...
class _RetraitsTabState extends State<RetraitsTab> {
  ...
  Future<void> _load() async {
    ...
    final centres = ElectionData.centresSuivi.map((c) => _LiveCentre(
      base: c, newRetraits: newByCode[c.codeCentre] ?? 0,
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      Expanded(child: ListView.builder(itemCount: filtered.length, itemBuilder: (ctx, i) {
        ...
        Color sc = pct >= 40 ? Colors.green : pct >= 25 ? Colors.orange : Colors.red;
        String st = pct >= 40 ? 'OK' : pct >= 25 ? 'ALERTE' : 'CRITIQUE';

        final allDaily = Map<String, int>.from(c.retraitsDailyData);
        lc.newDaily.forEach((date, v) { allDaily[date] = (allDaily[date] ?? 0) + v; });
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(12), children: [
      ...
      ...sorted.asMap().entries.map((entry) {
        ...
        Color color = pct >= 40 ? Colors.green : pct >= 25 ? Colors.orange : Colors.red;
